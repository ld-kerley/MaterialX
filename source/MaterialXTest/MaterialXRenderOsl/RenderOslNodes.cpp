...
class OslNodesShaderRenderTester : public RenderUtil::ShaderRenderTester
{
  public:
    explicit OslNodesShaderRenderTester(mx::ShaderGeneratorPtr shaderGenerator) :
...
// Renderer setup
void OslNodesShaderRenderTester::createRenderer(std::ostream& log)
{
...
// Renderer execution
bool OslNodesShaderRenderTester::runRenderer(const std::string& shaderName,
                                         mx::TypedElementPtr element,
                                         mx::GenContext& context,
                                         mx::DocumentPtr doc,
                                         std::ostream& log,
                                         ...
                                         mx::ImageVec* imageVec)
{
    ...
    if (element && doc)
    {
        ...
        for (const auto& options : optionsList)
        {
            ...
            std::string oslCommandString = shader->getSourceCode();

            oslCommandString  = "shader test_node test_node;\n";
            oslCommandString += "shader closure_passthrough closure_passthrough;\n";
            oslCommandString += "connect test_node.Out_Ci closure_passthrough.Cin;\n";
            ...
            if (testOptions.dumpGeneratedCode)
            {
                ...
                std::ofstream file;
                file.open(shaderPath + "_oslnodes.osl");
                file << oslCommandString;
                file.close();
            }
            ...
            try
            {
                ...
                if (outputs.size() > 0)
                {
                    ...
                    const std::string& sceneTemplateFile = "oslnodes_scene_template.xml";

                    _renderer->useOslCommandStrings(true);
                    _renderer->setOslCommandString(oslCommandString);
...
TEST_CASE("Render: OSL Nodes TestSuite", "[renderoslnodes]")
{
    ...
    OslNodesShaderRenderTester renderTester(mx::OslShaderGenerator::create());
